...
https://github.com/ethankruse/koi3278
"""
...
def mandel_agol(z,u1,u2,p0):
    """
    Adaptation of the Mandel & Agol transit light curve for quadratic limb
    darkening.
    Parameters
    ----------
    z : ndarray
        Impact parameters at which to evaluate the flux.
    u1 : float
        Linear limb darkening term
    u2 : float
        Quadratic limb darkening term
    p0 : float
        Radius ratio of the transiting object to the occulted object
    Returns
    -------
    fdark : ndarray
        Limb darkened flux at all points in z
    fflat : ndarray
        Flux without any limb darkening.
    """
    nz = size(z)
    lambdad = zeros(nz)
    etad = zeros(nz)
